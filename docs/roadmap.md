...
Status marks: ✅ done · 🔨 in progress · ⏳ planned
...
- ✅ **K1.5 — The pendrive.** `build/nova.hdd`: GPT + FAT16 ESP + BIOS-boot partition, built by
  ...
  (42 checks, byte-exact file round-trip), BIOS stages installed by `limine bios-install`.
...
- ✅ **K2 — Console.** PS/2 keyboard polling (ports 0x60/0x64) with correct controller
  ...
  the reference by `tests/test_font_ref.py`, full CPU exception handlers (IDT), format-correct
  16/24/32-bpp framebuffer writes, text console with scrolling + serial mirror, tiny shell
  (`help`, `clear`, `mem`, `ver`, `reboot`, `halt`). Interactive typing verified in QEMU via
  monitor `sendkey` (`tools/monitor-type.py`). CI selftest image (`novatest` cmdline → clean
  exit 33) plus regular-image prompt check.
